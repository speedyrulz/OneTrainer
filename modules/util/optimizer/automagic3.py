"""
Copied and adapted from ostris/ai-toolkit (MIT licence):
https://github.com/ostris/ai-toolkit/blob/main/toolkit/optimizers/automagic3.py

The control algorithm is unchanged. The adaptations are for OneTrainer's optimizer interface:

* Upstream registers its own post-accumulate-grad hooks for its ``fused`` mode. OneTrainer registers
  its own for "Fused Back Pass" and drives the optimizer through ``step_parameter``, so keeping
  upstream's hooks would update every parameter twice. They are dropped and ``step_parameter`` is
  implemented instead.
* With a fused back pass the trainer never calls ``step()``, so the pooled learning rate vote is
  flushed from ``zero_grad()`` instead - that is the one call the trainer always makes per step.
* Upstream's low-precision gradient accumulation buffer is dropped; OneTrainer accumulates gradients
  through autograd and handles bf16 stochastic rounding itself.
...
class Automagic3(torch.optim.Optimizer):
    ...
    In OneTrainer's normal mode, grads accumulate across backward passes and
    the update happens in ``.step()``. When OneTrainer's Fused Back Pass option
    is enabled, the trainer calls ``.step_parameter()`` from its own
    post-accumulate hooks so gradient reduction, clipping, and AMP handling stay
    consistent with the rest of OneTrainer's optimizer integrations.
    ...
    @torch.no_grad()
    def step_parameter(self, p: torch.Tensor, group: dict, i: int) -> None:
        self._update_param(p, group)

    @torch.no_grad()
    def zero_grad(self, set_to_none: bool = True) -> None:
        # AMP fused-back-pass skips step(); apply any pending group lr votes
        # before the trainer clears gradients.
        self._apply_group_votes()
