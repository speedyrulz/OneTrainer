...
class BaseOptimizerParamsWindowView:
    ...
    def build_dynamic_content(self, master, controller, optimizer_ui_state,
        ...
        KEY_DETAIL_MAP = {
            ...
            'polarity_history': {'title': 'Polarity History', 'tooltip': 'Automagic3: how many past update signs each element remembers (2-64). Every element votes only when its whole window agrees or perfectly alternates, and the group learning rate moves on the pooled result. A longer window is steadier but slower to react.', 'type': 'int'},
            'min_lr': {'title': 'Minimum LR', 'tooltip': 'Automagic3: hard floor for the adapted learning rate. At the default this is only an overflow guard; raise it to stop the controller going lower than you want.', 'type': 'float'},
            'max_lr': {'title': 'Maximum LR', 'tooltip': 'Automagic3: hard ceiling for the adapted learning rate. At the default this is only an overflow guard; lower it to cap how far the controller can climb.', 'type': 'float'},
